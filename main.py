...
def contains_stop_words(text: str) -> bool:
    ...
    # если есть разрешённый контекст — стоп-слова игнорируются
    if has_allowed_context(t):
        return False
...
def contains_keywords(text: str) -> bool:
    ...
    has_allowed = has_allowed_context(t)
    return has_trade and has_allowed
...
# ================= START =================
print("Bot started (STOP words conditional logic enabled)")
    ...
    try:
        ...
        for msg in reversed(messages):
            ...
            if contains_stop_words(raw):
                print("  ⛔ stop-word (no allowed context)")
                continue

            if not contains_keywords(raw):
                print("  ⛔ no trade / no allowed keywords")
                continue
