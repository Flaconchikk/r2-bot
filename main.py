...
UA_KEYWORDS = ["укр", "укра", "банк", "банки", "грн"]
USDT = ["usdt", "юсдт"]
...
def contains_keywords(text: str) -> bool:
    ...
    has_buy_sell = any(w in t for w in BUY_SELL)
    has_ua = any(w in t for w in UA_KEYWORDS)
    has_usdt = any(w in t for w in USDT)

    # OPTION A: allow 'укр' standalone
    return has_buy_sell and (has_ua or has_usdt)
...
# ================= START =================
print("Bot started (OPTION A: ukr allowed)")
...
while True:
    try:
        ...
        badge = "🔴 BUY" if trade_type == "buy" else "🟢 SELL" if trade_type == "sell" else "🟦 TRADE"
