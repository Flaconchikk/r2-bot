...
def contains_stop_words(text: str) -> bool:
    ...
    # если есть валюта — стоп-слова игнорируем
    if has_currency(t):
        return False
...
def contains_keywords(text: str) -> bool:
    ...
    has_curr = has_currency(t)

    # 🔒 строго: сделка + валюта
    return has_trade and has_curr
...
# ================= START =================
print("Bot started (STRICT currency mode enabled)")
    ...
    try:
        ...
        for msg in reversed(messages):
            ...
            if contains_stop_words(raw):
                print("  ⛔ stop-word (no currency)")
                continue

            if not contains_keywords(raw):
                print("  ⛔ no trade or no currency")
                continue
